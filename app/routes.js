...
module.exports = function(app, passport) {
            ...
            return new Promise((resolve, reject) => {
                ...
                ffmpeg.ffprobe(file, (error, videoInfo) => {
                    ...
                    ffmpeg()
                        ...
                        .outputOptions(
                            //'-preset', 'veryfast',
                            '-threads', '2') //Show PSNR measurements in output. Anything above 40dB indicates excellent fidelity
                        .videoCodec('libx264')
                        .size(`${((videoInfo.streams[0].width >= video_w) ? video_w : videoInfo.streams[0].width)}x?`).aspect('9:16').autopad()
                        .output(dest)
                        .outputFPS(29)
        ...
        const stripe = require('stripe')('pk_test_51IMfNYJsjY5mHcucDcfwwiS2RvBLMHVGBkTOzu63LIOMnma9OOuI6DnTQxBR3K3hzbCZ5Cs11XUK4mmXEfxUZJot00kRoOfk0G');
        ...
        app.post('/loja/login',
            ...
            function(req, res) {
                req.session.save(() => {
                    res.redirect((req.session.urlRedirect) ? req.session.urlRedirect : '/loja/dashboard');
                });
            });
        ...
        app.post('/loja/signup', passport.authenticate('local-signup', {
    ...
    app.get('/loja/order/:order_id', isLoggedIn, async function (req, res, next) {
        ...
        var order = await pool.query("SELECT * FROM orders WHERE store_id = ? AND order_cod=? LIMIT 0,1", [req.user.id, order_id])
        // var products = await pool.query("SELECT * FROM orders_products WHERE store_id = ? AND order_id=? LIMIT 0,1", [req.user.id, order[0].id])

        if (order[0].read == 0) {
            await pool.query('UPDATE orders SET orders.read=1 WHERE store_id = ? AND order_cod=?', [req.user.id, order_id])
        }

        var products_array = []
        var products = await pool.query("SELECT * FROM orders_products WHERE store_id = ? AND order_id=?", [req.user.id, order[0].id])
    ...
    app.post('/loja/api/orders', isLoggedIn, async function (req, res, next) {
        ...
        var orders = await pool.query("Select * From orders Where orders.store_id = ? AND (created BETWEEN ? AND ?) Order by created DESC", [req.user.id, start, end])
...
*/
    ...
    app.post('/loja/store', isLoggedIn, upload.single('file'), async (req, res, next) => {
        ...
        if (req.file) {
            ...
            await logofilehelper.compressImage(req.file, req.user.folder + '/logo')
                .then(newPath => {
                    // apagar imagem anterior
                    pool.query('UPDATE stores SET logo = ? WHERE id = ?', [newPath, req.user.id])
                    /*
    ...
    app.post('/loja/seller/delete', isLoggedIn, async function (req, res, next) {
        ...
        //console.log(url)
        if (url) {
            await pool.query("DELETE FROM sellers WHERE store_id = ? AND url = ?", [req.user.id, url])
    ...
    app.post('/loja/product/delete', isLoggedIn, async function (req, res, next) {
        ...
        if (id_product) {
            var product = await pool.query("DELETE FROM products WHERE store_id = ? AND product_id = ?", [req.user.id, id_product])

            if (product) {

                var images = await pool.query("SELECT product_id,name FROM media WHERE store_id = ? AND product_id = ? ORDER BY media.order ASC", [req.user.id, id_product])
                //apaga tudo do db
                await pool.query("DELETE FROM media WHERE store_id = ? AND product_id = ?", [req.user.id, id_product])
    ...
    app.post('/loja/categorie/delete', isLoggedIn, async function (req, res, next) {
        var id_categorie = req.body.id_categorie
        //console.log("CATID _>", id_categorie)
        if (id_categorie) {

            var categorie = await pool.query("DELETE FROM categories WHERE store_id = ? AND category_id = ?", [req.user.id, id_categorie])

            var products = await pool.query("SELECT * FROM products WHERE store_id = ? AND categorie_id = ?", [req.user.id, id_categorie])

            await asyncForEach(products, async (product) => {

                if (product) {
                    //console.log(product)
                    var images = await pool.query("SELECT * FROM media WHERE store_id = ? AND product_id = ? ORDER BY media.order ASC", [req.user.id, product.product_id])
                    //apaga tudo do db
                    await asyncForEach(images, async (image) => {
                        var filePath = `./public/media/${req.user.folder}/${image.name}`;
                        fs.unlinkSync(filePath);
                    ...
                    await pool.query("DELETE FROM media WHERE store_id = ? AND product_id = ?", [req.user.id, product.product_id])
                    await pool.query("DELETE FROM products WHERE store_id = ? AND categorie_id = ?", [req.user.id, id_categorie])
    ...
    app.get('/loja/variations', isLoggedIn, async function (req, res, next) {
        ...
        var tags_titles = []
        var tag_title = await pool.query("SELECT id,name, required, active FROM tags_title WHERE store_id = ? ORDER BY tags_title.order ASC", req.user.id)

        await asyncForEach(tag_title, async (title) => {
            var tags = await pool.query("SELECT * FROM tags WHERE store_id = ? AND tags_title = ? AND active = 1 ORDER BY tags.name", [req.user.id, title.id])
            tags_titles.push({
                "id": title.id,
                "name": title.name,
                "active": title.active,
    ...
    app.get('/loja/variations/create', isLoggedIn, async function (req, res, next) {
    ...
    app.get('/loja/variations/editor/:tag_title', isLoggedIn, async function (req, res, next) {

        var tag_title = req.params.tag_title
        var title = await pool.query("SELECT * FROM tags_title WHERE tags_title.store_id = ? AND tags_title.id = ? LIMIT 0,1", [req.user.id, tag_title])
    ...
    app.get('/loja/variations/:tag_id/tags/edit', isLoggedIn, async function (req, res, next) {

        var tag_id = req.params.tag_id
        var tags = await pool.query("SELECT * FROM tags WHERE tags.store_id = ? AND tags.id = ? LIMIT 0,1", [req.user.id, tag_id])
    ...
    app.post('/loja/variations/editor/', isLoggedIn, async function (req, res, next) {
        ...
        if (id) {
            ...
            await pool.query('UPDATE tags_title SET ? WHERE tags_title.store_id = ? AND tags_title.id = ? ', [data, req.user.id, id], function (err, result) {
            ...
            await pool.query('INSERT INTO tags_title SET ?', data, function (err, result) {
                res.redirect('/loja/variations')
            })
    ...
    app.post('/loja/tags/editor', isLoggedIn, async function (req, res, next) {
       ...
       // console.log(data, id)
        ...
        if (id) {
            await pool.query('UPDATE tags SET ? WHERE tags.store_id = ? AND tags.id = ? ', [data, req.user.id, id], function (err, result) {
    ...
    app.get('/loja/variations/:tag_title/tags', isLoggedIn, async function (req, res, next) {
        var tag_title = req.params.tag_title
        ...
        if (title[0]) {
            ...
            var tags = await pool.query("SELECT * FROM tags WHERE tags.store_id = ? AND tags.tags_title = ? ORDER BY tags.name ASC", [req.user.id, tag_title])
    ...
    app.get('/loja/variations/:tag_title/tags/create', isLoggedIn, async function (req, res, next) {
    ...
    app.get('/loja/categories', isLoggedIn, async function (req, res, next) {
        ...
        var categories = await pool.query("SELECT * FROM categories WHERE store_id = ? ORDER BY categories.name ASC", [req.user.id])
        var categorie_array = []
        ...
        await asyncForEach(categories, async (categorie) => {
            ...
            const ProdCount = await pool.query(`SELECT COUNT(*) AS Total FROM products WHERE store_id = ? AND (FIND_IN_SET('${categorie.category_id}', products.categorie_id))`, [req.user.id])
    ...
    app.get('/loja/categories/edit/:categoryid', isLoggedIn, async function (req, res, next) {


        var categorie = await pool.query("SELECT * FROM categories WHERE store_id = ? AND category_id = ?", [req.user.id, req.params.categoryid])
    ...
    app.post('/loja/categories/edit/:categoryid', isLoggedIn, async function (req, res, next) {
        ...
        if (categorie) {
            ...
            pool.query('UPDATE categories SET ? WHERE category_id = ?', [data, req.params.categoryid], function (err, result) {
    ...
    app.get('/loja/categories/create', isLoggedIn, async function (req, res, next) {
        res.render('admin/categories/categorie-add.ejs', {
            store: req.user
        });
    })

    app.post('/loja/categories/create', isLoggedIn, async function (req, res, next) {
        var name = req.body.name
    ...
    app.post('/loja/update/category-update', isLoggedIn, async function (req, res, next) {
        var id = req.body.id
        var status = req.body.status
        //req.user
        await pool.query("UPDATE categories SET active = ? WHERE id = ? AND store_id = ?", [status, id, req.user.id])
        res.json({
            status: 'updated'
        })
    })

    app.post('/loja/update/product-update', isLoggedIn, async function (req, res, next) {
        var id = req.body.id
        var status = req.body.status
        var action = req.body.action
        //req.user
        if (action == 'active') {
            await pool.query("UPDATE products SET active = ? WHERE product_id = ? AND store_id = ?", [status, id, req.user.id])

        } else if (action == 'featured') {
            await pool.query("UPDATE products SET featured = ? WHERE product_id = ? AND store_id = ?", [status, id, req.user.id])
    ...
    app.get('/loja/categories/:categoryid', isLoggedIn, async function (req, res, next) {
        ...
        var category_info = await pool.query("SELECT name, category_id FROM categories WHERE store_id = ? AND category_id = ?", [req.user.id, req.params.categoryid])

        const tags_titles = await pool.query("SELECT tags_title.name As title, tags_title.id, tags_title.required FROM tags INNER JOIN tags_title ON tags_title.id = tags.tags_title WHERE tags_title.store_id = ? AND tags_title.active ='1' GROUP BY  tags_title.name, tags_title.id ORDER BY tags_title.order ASC", [req.user.id])  
        ...
        const tags = await pool.query("SELECT * FROM tags WHERE store_id = ? AND active ='1'", [req.user.id])
        ...
        if (category_info[0]) {
            ...
            var product_info = await pool.query("SELECT * FROM products WHERE store_id = ? AND categorie_id REGEXP (?) ORDER BY name ASC", [req.user.id, category_info[0].category_id])
...
//myArray.findIndex(x => x.id === '45');
                ...
                const images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC LIMIT 0,3", [product.product_id])
                var arrayProd = {
                    "product_id": product.product_id,
                    "name": product.name,
                    "price": product.price,
    ...
    app.get('/loja/categories/:categoryid/product/create', isLoggedIn, async function (req, res, next) {

        var tags_titles = []
        var tag_title = await pool.query("SELECT id,name, required FROM tags_title WHERE store_id = ? AND active = 1", req.user.id)
        ...
        var category_from = await pool.query("SELECT name, category_id FROM categories WHERE store_id = ? AND category_id = ?", [req.user.id, req.params.categoryid])

        if (category_from[0]) {
            var category_info = await pool.query("SELECT name, category_id FROM categories WHERE store_id = ?", [req.user.id])
    ...
    app.post('/loja/:categoryid/create/product', isLoggedIn, async function (req, res, next) {
        //console.log(req.body)
    ...
    app.post('/loja/edit/product', isLoggedIn, async function (req, res, next) {
        ...
        await pool.query("UPDATE products SET ? WHERE product_id = ? AND store_id = ?", [data, product_id, req.user.id])
        //await pool.query('INSERT INTO products SET ?', data)
        //const LastInsert = order.insertId
        // res.redirect('/loja/categories/')
        ...
        res.redirect((req.session.urlCategories) ? req.session.urlCategories : '/loja/categories/');
    ...
    app.post('/loja/product/update-order', isLoggedIn, async function (req, res, next) {

        var product_id = req.body.product_id
        var orders = req.body.order
        //req.user

        var product_info = await pool.query("SELECT * FROM products WHERE store_id = ? AND product_id = ?", [req.user.id, product_id])
        //console.log(product_info)
        if (product_info[0]) {
            ...
            await asyncForEach(orders, async (order) => {
                ...
                await pool.query("UPDATE media SET media.order = ? WHERE name = ? AND product_id = ? AND store_id = ?", [order.i, order.file, product_id, req.user.id])
    ...
    app.get('/loja/product/:product_id', isLoggedIn, async function (req, res) {
        ...
        var product_info = await pool.query("SELECT * FROM products WHERE store_id = ? AND product_id = ?", [req.user.id, req.params.product_id])
        //console.log(product_info)
        if (product_info[0]) {
    ...
    app.post('/loja/upload', isLoggedIn, upload.single('file'), async (req, res, next) => {

        var this_product_id = req.body.product_id
        var count = req.body.count

        var product_info = await pool.query("SELECT id,product_id,name FROM products WHERE store_id = ? AND product_id = ?", [req.user.id, this_product_id])
    ...
    app.post('/loja/StoriesUpload', isLoggedIn, upload.single('file'), async (req, res, next) => {

       // console.log(req.file, req.user.folder)
        var count = req.body.count
        videohelper.compressVideo(req.file, req.user.folder)
            .then(newPath => {
                // Vamos continuar normalmente, exibindo o novo caminho
                //cadastrar a media no bando de dados
                var data = {
                ...
                pool.query('INSERT INTO stories SET ?', data, function (err, result) {
                    ...
                    let dest = `public/media/${req.user.folder}/stories/compressed_` + newPath
                    ...
                    queue.add(async () => {
                        await CompressVideo(path, dest).then(() => {
                            ...
                            if (getFilesizeInBytes(dest) > getFilesizeInBytes(path)) {
                                pool.query(`UPDATE stories SET stories.processed = 1 WHERE name = ?  AND store_id = ?`, [`${newPath}`, req.user.id])
                                fs.unlinkSync(dest);
                            } else {
                                pool.query(`UPDATE stories SET stories.processed = 1, name = ? WHERE name = ?  AND store_id = ?`, [`compressed_${newPath}`, `${newPath}`, req.user.id])
                                fs.unlinkSync(path);
...
*/
    ...
    app.post('/loja/BannersUpload', isLoggedIn, upload.single('file'), async (req, res, next) => {

        var count = req.body.count
        bannerhelper.compressImage(req.file, req.user.folder)
            .then(newPath => {
                // Vamos continuar normalmente, exibindo o novo caminho
                //cadastrar a media no bando de dados
                var data = {
    ...
    app.post('/loja/stories/update-order', isLoggedIn, async function (req, res, next) {
    ...
    app.post('/loja/story/update-url', isLoggedIn, async function (req, res, next) {
        var url = req.body.url
        var name = req.body.name
        ...
        await pool.query("UPDATE stories SET stories.url = ? , stories.button = ? WHERE name = ?  AND store_id = ?", [url, button, name, req.user.id])
    ...
    app.post('/loja/banner/update-order', isLoggedIn, async function (req, res, next) {
        var orders = req.body.order
        //req.user
        await asyncForEach(orders, async (order) => {
            //await pool.query("UPDATE media SET media.order = ? ", order.i)
            await pool.query("UPDATE banners SET banners.order = ? WHERE name = ?  AND store_id = ?", [order.i, order.file, req.user.id])
        })
        res.json({
            status: 'updated'
        })
    ...
    app.post('/loja/banner/update-url', isLoggedIn, async function (req, res, next) {
        var url = req.body.url
        var name = req.body.name
       ...
       // console.log(url, name)
        ...
        await pool.query("UPDATE banners SET banners.redirect = ? WHERE name = ?  AND store_id = ?", [url, name, req.user.id])
    ...
    app.post('/loja/media', isLoggedIn, async function (req, res) {
        ...
        var product_id = req.body.product_id
        //var count = req.body.count
        var image = await pool.query("SELECT product_id,name FROM media WHERE store_id = ? AND product_id = ? ORDER BY media.order ASC", [req.user.id, product_id])
        res.json({
            image
        })
    });
    ...
    app.post('/loja/StoriesMedia', isLoggedIn, async function (req, res) {
        ...
        var product_id = req.body.product_id
        //var count = req.body.count
        var image = await pool.query("SELECT * FROM stories WHERE store_id = ? ORDER BY stories.order ASC", [req.user.id])
        res.json({
            image
        })
    });
    ...
    app.post('/loja/subscrible', isLoggedIn, async function (req, res) {
        ...
        const tokenexist = await pool.query("SELECT token FROM notification WHERE token = ? LIMIT 0,1", token)
        if(!tokenexist[0]){
   ...
   res.json( tokens)
    ...
    app.post('/loja/BannersMedia', isLoggedIn, async function (req, res) {
        ...
        var product_id = req.body.product_id
        //var count = req.body.count
        var image = await pool.query("SELECT * FROM banners WHERE store_id = ? ORDER BY banners.order ASC", [req.user.id])
        res.json({
            image
        })
    });
    ...
    app.post('/loja/media/delete', isLoggedIn, async function (req, res) {
        ...
        var image = await pool.query("SELECT product_id,name FROM media WHERE store_id = ? AND name = ?", [req.user.id, req.body.name])

        //DELETE FROM NOME_DA_TABELA WHERE id = VALOR_DO_ID;
        await pool.query("DELETE FROM media WHERE store_id = ? AND name = ?", [req.user.id, req.body.name])
    ...
    app.post('/loja/tag/delete', isLoggedIn, async function (req, res) {
        ...
        var tag = await pool.query("SELECT id,name,tags_title FROM tags WHERE store_id = ? AND id = ?", [req.user.id, req.body.id])
        //console.log(banner)
        if (tag[0]) {
            //DELETE FROM NOME_DA_TABELA WHERE id = VALOR_DO_ID;
            await pool.query("DELETE FROM tags WHERE store_id = ? AND id = ?", [req.user.id, req.body.id])
    ...
    app.post('/loja/variations/delete', isLoggedIn, async function (req, res) {
        // render the page and pass in any flash data if it exists
        //req.body.id
        var tags_title = await pool.query("SELECT id,name FROM tags_title WHERE store_id = ? AND id = ?", [req.user.id, req.body.id])
        //console.log(banner)
        if (tags_title[0]) {
            //DELETE FROM NOME_DA_TABELA WHERE id = VALOR_DO_ID;
            await pool.query("DELETE FROM tags_title WHERE store_id = ? AND id = ?", [req.user.id, req.body.id])
    ...
    app.post('/loja/banner/delete', isLoggedIn, async function (req, res) {
        ...
        var banner = await pool.query("SELECT id,name FROM banners WHERE store_id = ? AND name = ?", [req.user.id, req.body.name])
        //console.log(banner)
        if (banner[0]) {
            //DELETE FROM NOME_DA_TABELA WHERE id = VALOR_DO_ID;
            await pool.query("DELETE FROM banners WHERE store_id = ? AND name = ?", [req.user.id, req.body.name])
    ...
    app.post('/loja/story/delete', isLoggedIn, async function (req, res) {
        ...
        var storie = await pool.query("SELECT id,name FROM stories WHERE store_id = ? AND name = ?", [req.user.id, req.body.name])

        if (storie[0]) {
            //DELETE FROM NOME_DA_TABELA WHERE id = VALOR_DO_ID;
            await pool.query("DELETE FROM stories WHERE store_id = ? AND name = ?", [req.user.id, req.body.name])
    ...
    app.get('/cart/:cod', async function (req, res, next) {
        try {
            ...
            if (store[0]) {
                ...
                //if (req.signedCookies['seller']) {
                   ...
                   /* if (seller_store != store[0].id) {
                        ...
                        var products = await pool.query("SELECT * FROM products WHERE active ='1' AND store_id = ? AND discount > 0 ORDER BY RAND() LIMIT 20", [store[0].id])
                        await asyncForEach(products, async (product) => {
                            //console.log(['joe', 'jane', 'mary'].includes('jane')); //true
                            var images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC LIMIT 0,1", [product.product_id])
                            var arrayProd = {
                        ...
                        var search_tags = []
                        const tags_title = await pool.query("SELECT * FROM tags_title WHERE store_id = ?  AND active ='1' ORDER BY tags_title.order ASC", [store[0].id])
                        ...
                        var products = await pool.query("SELECT * FROM products WHERE active ='1' AND store_id = ? AND featured = 1 ORDER BY RAND() LIMIT 20", [store[0].id])
                        await asyncForEach(products, async (product) => {
                            //console.log(['joe', 'jane', 'mary'].includes('jane')); //true
                            var images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC LIMIT 0,1", [product.product_id])
                            var arrayProd = {
                        ...
                        var products = await pool.query("SELECT * FROM products WHERE active ='1' AND store_id = ?  ORDER BY RAND() LIMIT 20", [store[0].id])
                        await asyncForEach(products, async (product) => {
                            //console.log(['joe', 'jane', 'mary'].includes('jane')); //true
                            var images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC LIMIT 0,1", [product.product_id])
                            var arrayProd = {
                        ...
                        var banners = await pool.query("SELECT * FROM banners WHERE store_id = ? ORDER BY banners.order ASC", [store[0].id])
               ...
               /* } else {
                ...
                res.status(404).send('Loja Não Encontrada, entre em contato com o suporte =/');
            }
    ...
    app.get('/:cod/stories/:file', async function (req, res) {
        ...
        const cod = req.params.cod
        const store = await pool.query("SELECT folder FROM stores WHERE cod = ?", [cod])
    ...
    app.get('/:cod/catalog', async function (req, res, next) {
        try {
            ...
            var products = await pool.query("SELECT * FROM products WHERE active ='1' AND store_id = ?", [store[0].id])
            await asyncForEach(products, async (product) => {
                //console.log(['joe', 'jane', 'mary'].includes('jane')); //true
                var images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC LIMIT 0,1", [product.product_id])
               /* var arrayProd = {
                ...
                <link>https://anyloja.com.br/${cod}/product/${product.product_id}</link>
                <image_link>https://anyloja.com.br/media/${cod}/${images[0].name}</image_link>
                <brand>${store[0].name}</brand>
                <condition>new</condition>
                <availability>${((product.active == 0) ? 'out of stock' : 'in stock')}</availability>
    ...
    app.get('/:cod/promotions', async function (req, res, next) {
        ...
        try {
            ...
            const products = await pool.query("SELECT * FROM products WHERE active ='1' AND discount > 0 AND store_id = ?  ORDER BY products.name ASC", store[0].id)
            await asyncForEach(products, async (product) => {
    ...
    app.get('/:cod/featured', async function (req, res, next) {
        ...
        try {
            ...
            const products = await pool.query("SELECT * FROM products WHERE active ='1' AND featured = 1 AND store_id = ?  ORDER BY products.name ASC", store[0].id)
            await asyncForEach(products, async (product) => {
    ...
    app.get('/order/:order_id', async function (req, res, next) {
        ...
        if (req.user) {
            ...
            var order = await pool.query("SELECT * FROM orders WHERE order_cod=? LIMIT 0,1", order_id)
            // var products = await pool.query("SELECT * FROM orders_products WHERE store_id = ? AND order_id=? LIMIT 0,1", [req.user.id, order[0].id])

            var store = await pool.query("SELECT id,logo,store,cod, name, address, bio, phone, min_order, instagram, facebook, folder, pixel  FROM stores WHERE id = ?", [order[0].store_id])

            var products_array = []
            var products = await pool.query("SELECT * FROM orders_products WHERE store_id = ? AND order_id=?", [order[0].store_id, order[0].id])
            await asyncForEach(products, async (product) => {
    ...
    app.get('/:cod/buscar', async function (req, res, next) {
        ...
        if (categorie) {
            const cat = await pool.query("SELECT name, category_id FROM categories WHERE category_id = ? AND store_id = ?", [categorie, store[0].id])
            if (cat[0]) {
        ...
        if (variations) {
            ...
            //pego os tagtitle disponiveis
            const tags_title2 = await pool.query("SELECT tags_title.name As title, tags_title.id, tags_title.required FROM tags INNER JOIN tags_title ON tags_title.id = tags.tags_title WHERE tags_title.store_id = ? AND tags.id IN (?) AND tags_title.active ='1' GROUP BY  tags_title.name, tags_title.id ORDER BY tags_title.order ASC", [store[0].id, productTags])

            await asyncForEach(tags_title2, async (tag_title) => {
                var mountTags = []
                const tags = await pool.query("SELECT * FROM tags WHERE store_id = ? AND active ='1' AND tags_title = ? AND tags.id IN (?)  ORDER BY CAST(tags.name AS UNSIGNED)", [store[0].id, tag_title.id, productTags])
                await asyncForEach(tags, async (tag) => {
                    productTags.find(el => {
                        if (el == tag.id) {
                            mountTags.push({
            ...
            const tags_title2 = await pool.query("SELECT tags_title.name As title, tags_title.id, tags_title.required FROM tags INNER JOIN tags_title ON tags_title.id = tags.tags_title WHERE tags_title.store_id = ? AND tags_title.active ='1' GROUP BY  tags_title.name, tags_title.id ORDER BY tags_title.order ASC", [store[0].id])

            await asyncForEach(tags_title2, async (tag_title) => {
                var mountTags = []
                const tags = await pool.query("SELECT * FROM tags WHERE store_id = ? AND active ='1' AND tags_title = ?  ORDER BY CAST(tags.name AS UNSIGNED)", [store[0].id, tag_title.id])
                await asyncForEach(tags, async (tag) => {
    ...
    app.post('/:cod/search', async function (req, res) {
        ...
        try {
            ...
            const products = await pool.query("SELECT * FROM products WHERE active ='1' AND store_id = ? AND name LIKE ?  ORDER BY products.name ASC", [store[0].id, `%${term}%`])
            await asyncForEach(products, async (product) => {
    ...
    app.get('/:cod/category/:category_id', async function (req, res, next) {
        ...
        try {
            ...
            // var products = await pool.query("SELECT * FROM products WHERE categorie_id = ?", [req.user.id, category_id])
            const store = await pool.query("SELECT id,logo,store,cod, name, address, bio, phone, min_order, instagram, facebook, folder, pixel  FROM stores WHERE cod = ?", [cod])
            const categorie = await pool.query("SELECT name, category_id FROM categories WHERE category_id = ? AND store_id = ?", [category_id, store[0].id])
            ...
            if (buscar) {
                var data = [
                    store[0].id, category_id, buscar
                ]
            } else {
                var data = [
                    store[0].id, category_id
                ]
            }

            const products = await pool.query(`SELECT * FROM products WHERE store_id = ?  AND active ='1' AND categorie_id REGEXP (?) ${((buscar) ? 'AND tags REGEXP(?)': '')} ORDER BY products.name ASC`, data)
    ...
    app.post('/:cod/send/whatsapp', async function (req, res) {
        ...
        var totalOrder = 0
        const cod = req.params.cod
        const store = await pool.query("SELECT id FROM stores WHERE cod = ?", [cod])


        if (store[0]) {
            const seller = await pool.query("SELECT * FROM sellers WHERE url = ?", [url])
            ...
            if (seller[0]) {
                ...
                await pool.query('INSERT INTO orders SET ?', data, async function (err, result) {
                    ...
                    await asyncForEach(products.cart.itens, async (produtc) => {
                        var DbProduct = await pool.query("SELECT * FROM products WHERE product_id = ?", [produtc.id])
                        ...
                        if (produtc.variation) {
                            ...
                            produtc.variation.forEach(variation => {
                                ...
                                variation_tpl.push(` ${variation.variation.charAt(0).toUpperCase() + variation.variation.substr(1).toLowerCase()}: ${variation.choice.charAt(0).toUpperCase() + variation.choice.substr(1).toLowerCase()}`)
                            });
                        }
                    ...
                    await pool.query('UPDATE orders SET ? WHERE order_cod = ?', [data, order_cod])
    ...
    app.get('/:cod/product/:product_id', async function (req, res, next) {
        ...
        try {
            ...
            const store = await pool.query("SELECT id,logo,store,cod, name, address, bio, phone, min_order, instagram, facebook, folder, pixel, installment  FROM stores WHERE cod = ?", [cod])
            const products = await pool.query("SELECT * FROM products WHERE active ='1' AND product_id = ? LIMIT 0,1", product_id)
            const images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC", product_id)
    ...
    app.post('/status/whatsapp', async function (req, res) {
        ...
        var data = {
            status: ((status == 'true') ? `Pedido Enviado com sucesso pelo cliente.` : `Segundo o cliente houve halgum erro ao enviar o pedido.`)
        };
        await pool.query('UPDATE orders SET ? WHERE order_cod = ?', [data, order_cod])
    ...
    app.post('/api/find/item', async function (req, res) {
        ...
        var this_product = await pool.query("SELECT * FROM products WHERE active ='1' AND product_id = ? LIMIT 0,1", [item_id])
        ...
        if (this_product[0]) {
            ...
            await asyncForEach(this_product, async (product) => {
                ...
                //console.log(['joe', 'jane', 'mary'].includes('jane')); //true
                const images = await pool.query("SELECT name FROM media WHERE product_id = ? ORDER BY media.order ASC", [product.product_id])
                var arrayProd = {
                    "product_id": product.product_id,
                    "name": product.name,
                    "price": product.price,
